...
export default function LeadPoolPage() {
  ...
  const analyzeAll = async (forceReanalyze = false) => {
    if (analyzing) return;
    setAnalyzing(true);
    setBulkMessage(null);
    try {
      ...
      if (forceReanalyze) analyzeParams.set('force', '1');
      const statsRes = await fetch(`/api/analyze?${analyzeParams}`);
      const statsData = await statsRes.json();
      const pendingIds: number[] = statsData.pendingIds || [];
      const catInfo = statsData.categoryInfo as { total: number; withWebsite: number; alreadyAnalyzed: number; noWebsite: number } | undefined;
      if (pendingIds.length === 0) {
        const infoText = catInfo
          ? `Alle ${catInfo.withWebsite} Leads mit Website sind bereits analysiert!${catInfo.noWebsite > 0 ? ` (${catInfo.noWebsite} Leads ohne Website übersprungen)` : ''}`
          : 'Alle Leads mit Website sind bereits analysiert!';
        setBulkMessage({ type: 'success', text: infoText });
        setAnalyzing(false);
        setTimeout(() => setBulkMessage(null), 5000);
        return;
      }
      // Show info about how many leads qualify
      if (catInfo && catInfo.total > pendingIds.length && !forceReanalyze) {
        const parts: string[] = [];
        if (catInfo.noWebsite > 0) parts.push(`${catInfo.noWebsite} ohne Website`);
        if (catInfo.alreadyAnalyzed > 0) parts.push(`${catInfo.alreadyAnalyzed} bereits analysiert`);
        const reason = parts.length > 0 ? ` (${parts.join(', ')})` : '';
        setBulkMessage({ type: 'success', text: `${pendingIds.length} von ${catInfo.total} Leads werden analysiert${reason}` });
      }
          ...
          <div className="flex items-center gap-2">
            {analyzing && analyzeProgress ? (
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => analyzeAll(false)} disabled={analyzing} className="px-4 py-2 rounded-xl text-sm font-medium flex items-center gap-2 transition-all bg-elvora-primary/20 text-elvora-primary hover:bg-elvora-primary/30 border border-elvora-primary/20 disabled:opacity-50">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>
                  Websites analysieren
                </button>
                <button onClick={() => analyzeAll(true)} disabled={analyzing} className="px-3 py-2 rounded-xl text-sm font-medium flex items-center gap-1.5 transition-all bg-white/5 text-elvora-text-muted hover:bg-white/10 hover:text-white border border-white/10 disabled:opacity-50" title="Alle erneut analysieren (auch bereits analysierte)">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <h3 className="text-sm font-semibold text-white">Gefundene E-Mails</h3>
                <p className="text-[11px] text-elvora-text-dim">
                  {emailCount > 0
                    ? `${emailCount} E-Mail${emailCount !== 1 ? 's' : ''} aus Website-Analyse`
                    : 'Noch keine E-Mails — Websites analysieren um E-Mails zu extrahieren'}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {emailCount > 0 && (
                <span className="px-2 py-0.5 rounded-md text-[11px] font-bold bg-elvora-primary/15 text-elvora-primary">
                  {emailCount}
                </span>
              )}
              <svg className={`w-4 h-4 text-elvora-text-dim transition-transform ${showEmailPanel ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {emailCount > 0 ? (
                <>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs text-elvora-text-dim">{emailCount} E-Mail{emailCount !== 1 ? 's' : ''}</span>
                    <button
                      onClick={exportEmailsTxt}
                      className="px-3 py-1.5 rounded-lg bg-elvora-primary/15 text-elvora-primary text-[11px] font-semibold border border-elvora-primary/20 hover:bg-elvora-primary/25 transition-all flex items-center gap-1.5"
                    >
                  ...
                  <div className="max-h-48 overflow-y-auto space-y-1 custom-scrollbar">
                    {leadsWithEmail.map(lead => (
                      <div key={lead.id} className="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-white/[0.03] group">
                        <div className="flex items-center gap-2 min-w-0">
                          <svg className="w-3.5 h-3.5 text-elvora-primary/60 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                          </svg>
                          <a href={`mailto:${lead.email}`} className="text-xs text-elvora-text-muted hover:text-elvora-primary transition-colors truncate">
                            {lead.email}
                          </a>
                        </div>
                        <span className="text-[10px] text-elvora-text-dim truncate ml-2 max-w-[120px] opacity-0 group-hover:opacity-100 transition-opacity">
                          {lead.name}
                        </span>
                      </div>
                    ))}
                ...
                <div className="py-6 text-center">
                  ...
                  <p className="text-xs text-elvora-text-dim">Noch keine E-Mails vorhanden.</p>
                  <p className="text-[11px] text-elvora-text-dim/60 mt-1">Klicke auf &quot;Websites analysieren&quot; um E-Mails zu extrahieren.</p>
                </div>
              )}
            </div>
          )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {analyzing && analyzeProgress ? (
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => analyzeAll(false)} disabled={analyzing} className="px-3 py-2 rounded-xl text-xs font-medium flex items-center gap-2 transition-all bg-elvora-primary/20 text-elvora-primary hover:bg-elvora-primary/30 border border-elvora-primary/20 disabled:opacity-50">
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>
                Analysieren
              </button>
              <button onClick={() => analyzeAll(true)} disabled={analyzing} className="px-2.5 py-2 rounded-xl text-xs font-medium flex items-center gap-1 transition-all bg-white/5 text-elvora-text-muted hover:bg-white/10 hover:text-white border border-white/10 disabled:opacity-50" title="Alle erneut analysieren">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <p className="text-[11px] text-elvora-text-dim">
                {emailCount > 0
                  ? `${emailCount} E-Mail${emailCount !== 1 ? 's' : ''} aus Website-Analyse`
                  : 'Noch keine E-Mails gefunden — Websites analysieren um E-Mails zu extrahieren'}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="border-t border-white/5 px-4 py-3">
            {emailCount > 0 ? (
              ...
              <div className="py-6 text-center">
                ...
                <p className="text-xs text-elvora-text-dim">Noch keine E-Mails vorhanden.</p>
                <p className="text-[11px] text-elvora-text-dim/60 mt-1">Klicke auf &quot;Analysieren&quot; um E-Mails von Websites zu extrahieren.</p>
              </div>
            )}
          </div>
        )}
