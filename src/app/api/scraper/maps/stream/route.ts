...
async function scrapeEmailFromWebsite(websiteUrl: string): Promise<{ email: string | null; phone: string | null }> {
  const domain = getDomain(websiteUrl);
  if (domainEmailCache.has(domain)) {
    return { email: domainEmailCache.get(domain) || null, phone: null };
  }
  try {
