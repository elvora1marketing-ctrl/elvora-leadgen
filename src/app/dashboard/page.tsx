    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={triggerScan}
          disabled={scanning}
          className="px-4 py-2 rounded-lg bg-elvora-primary text-white text-sm font-medium hover:bg-elvora-primary-dark transition-colors disabled:opacity-50 flex items-center gap-2"
        >
          {scanning ? (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-5">
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
            {[
              {
                label: 'Zu prüfen',
                value: stats ? fmt(stats.leads.pending) : '–',
                sub: stats ? `+${stats.leads.todayNew} heute` : '...',
                subColor: 'text-elvora-success',
                iconColor: 'text-elvora-warning',
                iconBg: 'bg-elvora-warning/10',
                icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />,
              },
              {
                label: 'Qualifiziert',
                value: stats ? fmt(stats.leads.qualified) : '–',
                sub: stats ? `+${stats.leads.weekQualified} Woche` : '...',
                subColor: 'text-elvora-success',
                iconColor: 'text-elvora-success',
                iconBg: 'bg-elvora-success/10',
                icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />,
              },
              {
                label: 'Öffnungsrate',
                value: stats ? `${stats.email.openRate}%` : '–',
                sub: stats ? `${stats.email.opened} / ${stats.email.totalSent} Mails` : '...',
                subColor: 'text-elvora-text-dim',
                iconColor: 'text-elvora-pink',
                iconBg: 'bg-elvora-pink/10',
                icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />,
              },
              {
                label: 'Pipeline',
                value: stats ? `${fmtEur(stats.pipeline.activeValue)}€` : '–',
                sub: stats ? `${stats.pipeline.activeDeals} aktive Deals` : '...',
                subColor: 'text-elvora-text-dim',
                iconColor: 'text-elvora-purple-light',
                iconBg: 'bg-elvora-primary/10',
              ...
              <div key={i} className="card rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className={`w-9 h-9 rounded-lg ${card.iconBg} flex items-center justify-center`}>
                    <svg className={`w-[18px] h-[18px] ${card.iconColor}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      {card.icon}
                    </svg>
                  </div>
                  <span className="text-[11px] text-elvora-text-dim font-medium">{card.label}</span>
                </div>
                <div className="text-2xl font-semibold text-elvora-text stat-number">{card.value}</div>
                <div className={`text-[11px] ${card.subColor} mt-1`}>{card.sub}</div>
              </div>
            ))}
          </div>
          ...
          {stats && (
            <div className="card rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-sm font-semibold text-elvora-text">Sales Funnel</h2>
                  <p className="text-[11px] text-elvora-text-dim mt-0.5">Conversion pro Stufe</p>
                </div>
                {stats.pipeline.wonValue > 0 && (
                  <div className="text-right">
                    <div className="text-lg font-semibold text-elvora-success stat-number">{fmt(stats.pipeline.wonValue)}€</div>
              ...
              <div className="space-y-2.5">
                {funnelSteps.map((step, i) => {
                  const colors = ['bg-elvora-text-dim/30', 'bg-elvora-purple/50', 'bg-elvora-warning/50', 'bg-elvora-pink/50', 'bg-elvora-success/50'];
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-7 bg-elvora-bg-alt rounded-md overflow-hidden relative">
                        <div
                          className={`h-full ${colors[i]} rounded-md flex items-center px-3`}
                          style={{ width: `${width}%` }}
                        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {engDist && engTotal > 0 && (
              <div className="card rounded-xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-sm font-semibold text-elvora-text">Engagement</h2>
                ...
                <div className="flex items-center gap-6">
                  <div className="relative w-28 h-28 flex-shrink-0">
                    <svg viewBox="0 0 36 36" className="w-28 h-28 -rotate-90">
                      {(() => {
                        const segments = [
                          ...
                          { pct: engDist.cold / engTotal * 100, color: '#2c3546' },
                        ];
                        let offset = 0;
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-xl font-semibold text-elvora-text stat-number">{engTotal}</span>
                  ...
                  <div className="space-y-2.5 flex-1">
                    {[
                      ...
                      { label: 'Warm', count: engDist.warm, color: 'bg-elvora-accent' },
                      { label: 'Cool', count: engDist.cool, color: 'bg-elvora-purple' },
                      { label: 'Kalt', count: engDist.cold, color: 'bg-elvora-border-light' },
                    ].map(item => (
                      <div key={item.label} className="flex items-center justify-between">
                        ...
                        <span className="text-xs font-medium text-elvora-text">{item.count}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {stats && stats.audits.total_audits > 0 && (
              <div className="card rounded-xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-sm font-semibold text-elvora-text">Audit Performance</h2>
                ...
                <div className="flex items-end justify-around h-28 gap-4 mb-3">
                  {[
                    { value: stats.audits.total_audits, label: 'Erstellt', color: 'bg-elvora-purple/40', max: Math.max(stats.audits.total_audits, stats.audits.total_views, stats.audits.total_cta_clicks) || 1 },
                    { value: stats.audits.total_views, label: 'Aufrufe', color: 'bg-elvora-accent/40', max: Math.max(stats.audits.total_audits, stats.audits.total_views, stats.audits.total_cta_clicks) || 1 },
                    { value: stats.audits.total_cta_clicks, label: 'CTA Klicks', color: 'bg-elvora-success/40', max: Math.max(stats.audits.total_audits, stats.audits.total_views, stats.audits.total_cta_clicks) || 1 },
                  ].map((bar, i) => (
                    <div key={i} className="flex flex-col items-center flex-1">
                      <span className="text-xs font-semibold text-elvora-text mb-1">{bar.value}</span>
                      <div
                        className={`w-full max-w-[48px] ${bar.color} rounded-t-md sparkline-bar`}
          ...
          {stats && stats.hotLeads.length > 0 && (
            <div className="card rounded-xl p-5">
              <div className="flex items-center gap-2.5 mb-4">
                ...
                <div>
                  <h2 className="text-sm font-semibold text-elvora-text">Hot Leads</h2>
                  <span className="text-[10px] text-elvora-text-dim">Haben deinen Audit angesehen</span>
                </div>
              </div>
              <div className="space-y-1.5">
                {stats.hotLeads.map((lead) => (
                  <Link
                    key={lead.id}
                    href={`/crm/${lead.id}`}
                    className="flex items-center justify-between p-3 rounded-lg hover:bg-elvora-bg-alt transition-colors group"
                  >
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      <div className="w-8 h-8 rounded-full bg-red-500/15 flex items-center justify-center text-red-400 text-xs font-semibold flex-shrink-0">
                        {lead.score}
                      </div>
                      <div className="min-w-0">
                        <div className="text-sm font-medium text-elvora-text truncate group-hover:text-elvora-purple-light transition-colors">{lead.name}</div>
                        <div className="text-xs text-elvora-text-dim flex items-center gap-2">
                          <span>{lead.city}</span>
                          {lead.audit_views > 0 && <span className="text-elvora-accent">{lead.audit_views}x gesehen</span>}
                          {lead.cta_clicks > 0 && <span className="text-red-400 font-medium">{lead.cta_clicks}x CTA</span>}
                        </div>
                      </div>
                    </div>
                    {lead.phone && (
                      <a
                        ...
                        className="ml-2 w-8 h-8 rounded-lg bg-elvora-success/10 flex items-center justify-center text-elvora-success hover:bg-elvora-success/20 transition-colors flex-shrink-0"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="card rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h2 className="text-sm font-semibold text-elvora-text">Auto Follow-Ups</h2>
                    <p className="text-[10px] text-elvora-text-dim">{stats.followUps.sentThisWeek} diese Woche gesendet</p>
                  </div>
                </div>
                {stats.followUps.dueNow > 0 && (
                  <span className="px-2.5 py-1 rounded-full bg-elvora-warning/15 text-elvora-warning text-[10px] font-semibold">
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                {[
                  ...
                  <div key={i} className="text-center p-3 rounded-lg bg-elvora-bg-alt">
                    <div className={`text-lg font-semibold ${s.color} stat-number`}>{s.value}</div>
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between text-xs py-1.5">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="w-5 h-5 rounded-md bg-elvora-accent/15 text-elvora-accent text-[10px] font-semibold flex items-center justify-center flex-shrink-0">
                            {fu.step}
                          </span>
          ...
          {stats && stats.recentScans.length > 0 && (
            <div className="card rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-semibold text-elvora-text">Letzte Scans</h2>
                ...
                <table className="w-full min-w-[400px]">
                  <thead>
                    <tr className="border-b border-elvora-border">
                      <th className="text-left text-[10px] text-elvora-text-dim uppercase tracking-wider pb-3 font-medium">Keyword</th>
                      <th className="text-left text-[10px] text-elvora-text-dim uppercase tracking-wider pb-3 font-medium">Stadt</th>
                      <th className="text-right text-[10px] text-elvora-text-dim uppercase tracking-wider pb-3 font-medium">Gefunden</th>
                      <th className="text-right text-[10px] text-elvora-text-dim uppercase tracking-wider pb-3 font-medium">Neu</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-elvora-border/50">
                    {stats.recentScans.map((scan, i) => (
                      <tr key={i} className="hover:bg-elvora-bg-alt transition-colors">
                        <td className="py-3 text-sm text-elvora-text font-medium">{scan.keyword}</td>
                        <td className="py-3 text-sm text-elvora-text-muted">{scan.city}</td>
                        <td className="py-3 text-sm text-elvora-text font-medium text-right">{scan.leads_found || 0}</td>
                        <td className="py-3 text-sm text-elvora-success font-medium text-right">+{scan.leads_new || 0}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="space-y-5">
          ...
          <div className="card rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-elvora-text">Kalender</h2>
              <span className="text-xs text-elvora-text-muted">{monthNames[today.getMonth()]}</span>
            </div>
                ...
                return (
                  <div
                    ...
                    className={`text-xs py-1.5 rounded-md transition-colors ${
                      isToday
                        ? 'bg-elvora-primary text-white font-semibold'
                        : d.current
                          ? 'text-elvora-text-muted hover:bg-elvora-bg-alt cursor-pointer'
                          : 'text-elvora-text-dim/30'
                    }`}
                  >
                    {d.day}
                  </div>
          ...
          {(tasks.length > 0 || taskCounts.open > 0) && (
            <div className="card rounded-xl p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <h2 className="text-sm font-semibold text-elvora-text">Aufgaben</h2>
                  {taskCounts.overdue > 0 && (
                    <span className="w-5 h-5 rounded-full bg-red-500/15 text-red-400 text-[10px] font-semibold flex items-center justify-center">
                      {taskCounts.overdue}
                    </span>
                  )}
                </div>
              ...
              {tasks.length === 0 ? (
                ...
                <div className="space-y-0.5">
                  {tasks.slice(0, 5).map(t => {
                    ...
                    return (
                      <div key={t.id} className="flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-elvora-bg-alt group transition-colors">
                        <button
                          onClick={() => toggleTask(t.id)}
                          className="w-4 h-4 rounded border border-elvora-border-light hover:border-elvora-purple flex-shrink-0 transition-colors"
                        />
                        <span className="text-[13px] text-elvora-text truncate flex-1">{t.title}</span>
                        {t.due_date && (
          ...
          {stats && stats.inbox.unread > 0 && (
            <div className="card rounded-xl p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <h2 className="text-sm font-semibold text-elvora-text">Neue Antworten</h2>
                  <span className="w-5 h-5 rounded-full bg-elvora-pink/15 text-elvora-pink text-[10px] font-semibold flex items-center justify-center">
                    {stats.inbox.unread}
                  </span>
                </div>
              ...
              <div className="space-y-1.5">
                {stats.inbox.recent.filter(r => !r.is_read).slice(0, 3).map(reply => (
                  <Link
                    key={reply.id}
                    href="/inbox"
                    className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-elvora-bg-alt transition-colors"
                  >
                    <div className="w-8 h-8 rounded-full bg-elvora-primary/15 flex items-center justify-center text-elvora-purple-light text-[11px] font-semibold flex-shrink-0">
                      {(reply.lead_name || reply.from_name || reply.from_email)[0].toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-[13px] font-medium text-elvora-text truncate">
          ...
          {stats && stats.engagement.topLeads.length > 0 && (
            <div className="card rounded-xl p-5">
              <h2 className="text-sm font-semibold text-elvora-text mb-3">Top Engaged</h2>
              <div className="space-y-1">
                {stats.engagement.topLeads.slice(0, 4).map((lead) => (
                  <Link
                    key={lead.id}
                    href={`/crm/${lead.id}`}
                    className="flex items-center gap-3 p-2 rounded-lg hover:bg-elvora-bg-alt transition-colors group"
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-semibold flex-shrink-0 ${
                      lead.engagement_score >= 50 ? 'bg-red-500/15 text-red-400' : 'bg-elvora-accent/15 text-elvora-accent'
                    }`}>
                      {lead.engagement_score}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-[13px] font-medium text-elvora-text truncate group-hover:text-elvora-purple-light transition-colors">{lead.name}</div>
                    ...
                    {lead.phone && (
                      <a
                        href={`tel:${lead.phone}`}
                        onClick={e => e.stopPropagation()}
                        className="w-7 h-7 rounded-lg bg-elvora-success/10 flex items-center justify-center text-elvora-success hover:bg-elvora-success/20 transition-colors flex-shrink-0"
                      >
          ...
          <div className="grid grid-cols-2 gap-3">
            <Link href="/scraper" className="card rounded-xl p-4 card-hover group text-center">
              <div className="w-9 h-9 rounded-lg bg-elvora-primary/10 flex items-center justify-center mx-auto mb-2">
                <svg className="w-[18px] h-[18px] text-elvora-purple-light" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
              </div>
              <span className="text-xs text-elvora-text-muted font-medium">Maps Scraper</span>
            </Link>
            <Link href="/gmb-audit" className="card rounded-xl p-4 card-hover group text-center">
              <div className="w-9 h-9 rounded-lg bg-elvora-pink/10 flex items-center justify-center mx-auto mb-2">
                <svg className="w-[18px] h-[18px] text-elvora-pink" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
              </div>
              <span className="text-xs text-elvora-text-muted font-medium">GMB Audit</span>
            </Link>
          </div>
        </div>
      </div>
